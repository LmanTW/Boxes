//Get Target
export default (operation, boxes, systemBoxes) => {
  if (operation.target[0] === undefined) return { error: true, content: 'Missing Target', line: operation.line }
    ...
    if (systemBoxes[operation.target[skip+1].value] !== undefined) return { error: true, content: `Box Named "${operation.target[skip+1].value}" Already Exist [Environment]`, line: operation.target[skip+1].line, start: operation.target[skip+1].start }
    if (boxes[operation.target[skip+1].value] !== undefined) return { error: true, content: `Box Named "${operation.target[1].value}" Already Exist`, line: operation.target[skip+1].line, start: operation.target[skip+1].start }
    ...
    if (systemBoxes[operation.target[0].value] !== undefined) return { error: true, content: `The Box Is Locked "${operation.target[0].value}" [Environment]`, line: operation.target[0].line, start: operation.target[0].start }
    if (boxes[operation.target[0].value] === undefined) return { error: true, content: `Box Not Found: "${operation.target[0].value}"`, line: operation.target[0].line, start: operation.target[0].start }
    if (boxes[operation.target[0].value].lock) return { error: true, content: `The Box Is Locked: "${operation.target[0].value}"`, line: operation.target[0].line, start: operation.target[0].start }
